...
def callback(data):
	...
	#239 = ganz links
	# 120 = mitte
	abstandRechts = data.ranges[0]
	abstandMitte = data.ranges[120]
	abstandLinks = data.ranges[239]
	if abstandRechts >= abstandMitte:
		if abstandRechts >= abstandLinks:
			#move to right
			rotate(45, True)
			moveStraight(abstandRechts/2)
		else:
			#move to left
			rotate(45, False)
			moveStraight(abstandLinks/2)
	else:
		if abstandMitte >= abstandLinks:
			#move straight
			moveStraight(abstandMitte/2)
		else:
			# move left
			rotate(45, False)
			moveStraight(abstandLinks/2)
